...
#SBATCH --output=/global/scratch/users/%u/syk/logs/kktz_kbe_%j.out
#SBATCH --error=/global/scratch/users/%u/syk/logs/kktz_kbe_%j.err
...
ALPHA_I=${ALPHA_I:-0.0}
ALPHA_F=${ALPHA_F:-$ALPHA_I}
TAG="kktz_kbe_J_$(safe $J_I)_to_$(safe ${J_F:-$J_I})_alpha_$(safe $ALPHA_I)_to_$(safe $ALPHA_F)_mu_$(safe $MU_I)_to_$(safe $MU_F)_b_$(safe $BETA)_dt_$(safe $DT)"
...
echo "Job $SLURM_JOB_ID: KKTZ KBE J_i=$J_I J_f=$J_F alpha_i=$ALPHA_I alpha_f=$ALPHA_F mu_i=$MU_I mu_f=$MU_F beta=$BETA dt=$DT"
echo "t_pre_factor=$T_PRE_FACTOR t_post_factor=$T_POST_FACTOR"
echo "Output: $OUT_DIR"

CMD=(python3 -u "$WORK_DIR/kktz_batch_tools2.py" kbe-one
    --J-i "$J_I"
    --J-f "$J_F"
    --alpha-i "$ALPHA_I"
    --alpha-f "$ALPHA_F"
