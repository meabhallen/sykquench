...
def build_kernel_R_mat(
    ...
    tau_1d = np.linspace(0.0, rel_max, 4 * len(t) + 1)
    K_R_tau_1d = omega_to_time(K_R_w, omega, tau_1d)
    K_R_of_tau = interp1d(tau_1d, K_R_tau_1d, kind="cubic", fill_value="extrapolate")
    tau_mat = t[:, None] - t[None, :]
    K_R_mat = np.zeros_like(tau_mat, dtype=complex)
    lower = tau_mat >= 0.0
    K_R_mat[lower] = K_R_of_tau(tau_mat[lower])
    return K_R_mat
...
def solve_equilibrium_greater_real_time(
    ...
    """
    Returns: omega_real, A, t_grid, F_t, Ggt_t, GR_w, K_R_w.
    ...
    for it in range(it_start, max_iter):
        ...
        if it % verbose_every == 0 or delta_F < tol:
            print(
    ...
    return omega_real, A, t_grid, F_t, Ggt_t, GR_w, K_R_w
...
def rhs_t1(
    ...
    never alongside Sigma^> in the I2 term below.
    """
    k = np.arange(i + 1)
...
def rhs_t2(
    ...
    exactly the K^A(t_k,t_j) contribution needed here.
    """
    k = np.arange(i + 1)
    w = trap_weights(len(k), dt)
    G_R = G[i, k] + G[k, i]
...
def run_equilibrium_one(
    ...
    *,
    ...
    try:
        omega_real, A, t_grid, F_t, Ggt_t, GR_w, K_R_w = solve_equilibrium_greater_real_time(
            J2=J2,
            J4=J4,
            beta=beta,
            omega_max=omega_max,
