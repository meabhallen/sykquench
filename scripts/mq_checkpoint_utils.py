"""Checkpointing + lean-output utilities for MQ/coupled-SYK KBE runs.

Drop this next to mq_kbe_patch.py on Lawrencium.
"""
...
def save_kbe_lean(
    ...
    *,
    ...
    meta = {
        ...
        "saved_full_G": False,
        "notes": "Lean MQ KBE output: post-quench equal-time observables and O1 only. Use checkpoint file to resume full two-time evolution.",
    }
    if extra_metadata:
...
def solve_equilibrium_mq_matrix_F_checkpointed(
    J,
    mu,
    beta,
    omega_max=8.0,
    ...
    """Checkpointed version of solve_equilibrium_mq_matrix_F from mq_kbe_patch_v4.py."""
    if t_max is None:
    ...
    def _checkpoint_then_exit(signum: int, _frame: Any) -> None:
        ...
        if ckpt is not None and checkpoint_state["it_done"] >= 0:
            ...
            print(
                f"\n[{signal_name}] MQ equilibrium checkpoint saved at "
                f"iter={checkpoint_state['it_done']}; exiting for requeue.",
                flush=True,
            )
        else:
            print(
                f"\n[{signal_name}] No completed MQ equilibrium iteration to "
                "checkpoint; exiting.",
                flush=True,
            )
        # SystemExit is deliberately not caught by callers' `except Exception`,
        # so an interrupted run is not marked as failed.
    ...
    for it in range(start_it, max_iter):
        ...
        if need_dab:
            last_dab = calc_kbe_d_ab_mq_equilibrium(
                t=t,
                Ggt=Ggt_current,
                J=J,
                mu=mu,
        ...
        if verbose_every and (it % verbose_every == 0 or delta_ok):
            ...
            if last_dab is not None and need_dab:
                msg += f"  KBE d_ab^{kbe_dab_power:g}={np.asarray(last_dab)**kbe_dab_power}  max={last_dab_sqrt_max:.3e} tol={dab_tol:.3e}"
    ...
    if converged and ckpt is not None and ckpt.exists():
        ckpt.unlink()
        print("MQ equilibrium checkpoint removed (converged).")
    elif not converged and ckpt is not None and checkpoint_state["it_done"] >= 0:
        print(
            f"MQ equilibrium checkpoint retained at iter={checkpoint_state['it_done']} "
...
def evolve_mq_kbe_matrix_checkpointed(
    t_eq,
    Ggt_eq,
    *,
    J_i=1.0,
    ...
    """Checkpointed version of evolve_mq_kbe_matrix_v2.
    ...
    def _checkpoint_then_exit(signum: int, _frame: Any) -> None:
        ...
        if ckpt is not None and n_done > n0:
            ...
            print(
                f"\n[{signal_name}] MQ KBE checkpoint saved at "
                f"n={n_done}, t={t[n_done]:.6g}; exiting for requeue.",
                flush=True,
            )
        else:
            print(
                f"\n[{signal_name}] No completed MQ KBE step beyond the "
    ...
    if ckpt is not None and ckpt.exists():
        ckpt.unlink()
        print("MQ KBE checkpoint removed (run complete).")
