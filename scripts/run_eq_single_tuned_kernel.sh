...
TAG="eq_J4_$(safe $J4)_b_$(safe $BETA)_lam_$(safe $KERNEL_LAMBDA)_c_$(safe $KERNEL_C)_L_$(safe $KERNEL_CUTOFF)_dt_$(safe $DT)_om_$(safe $OMEGA_MAX)_Nw_${NW}"
OUT_DIR="eq_tuned_runs/$TAG"

echo "Job $SLURM_JOB_ID: J4=$J4 beta=$BETA lambda=$KERNEL_LAMBDA c=$KERNEL_C Lambda=$KERNEL_CUTOFF dt=$DT omega_max=$OMEGA_MAX Nw=$NW tol=$TOL"
echo "Output: $OUT_DIR"
...
# ── Run ───────────────────────────────────────────────────────────────────────
python3 -u "$WORK_DIR/syk_batch_tools2_tuned_kernel.py" eq-one \
    --J4        "$J4"        \
    --beta      "$BETA"      \
    --dt        "$DT"        \
    --omega-max "$OMEGA_MAX" \
